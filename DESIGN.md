...
Where two enabled tokens match at the same position of the content (one
token is a prefix of another, e.g. `@@A@@` and `@@A@@B@@`), the longest
matching token wins. Matching then continues after the replaced bytes;
tokens are never matched inside substituted values. This is observable
behavior and therefore part of the format contract.
...
-   `json-patch` when `--text-patch` is not given, the rule permits
    ...
    rewrite keys the user did not touch. This gate applies to automatic
    selection only: when the target's existing override for the file
    already declares `json-patch`, the user has chosen canonicalization
    (every materialization performs it), so `repatch` MUST honour that
    choice regardless of the base's form. If the two decoded documents
